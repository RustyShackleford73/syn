...
class Scene:
    ...
    def detection_to_global(self,*cam,motion_prior=False):
        ...
        for i in cams:
            ...
            # detect = self.cameras[i].undist_point(self.detections[i][1:]) if (self.detections[i][1:]).shape!=0 and self.settings['undist_points'] else self.detections[i][1:]
...
class Camera:
    ...
    def __init__(self,**kwargs):
        ...
        self.joint_id = kwargs.get('joint_id')
    ...
    def undist_point(self,points):
        ...
        #        pickle.dump([points, src, self.K, self.d], f)
        #     exit()
...
def create_scene(path_input):
    ...
    # Load detections
    joint_id = config['optional inputs']['joint_id']
    path_detect = config['necessary inputs']['path_detections']
    flight.numCam = len(path_detect)
    for i in path_detect:
        # detect = np.loadtxt(i,usecols=(0,1,2))[:flight.settings['num_detections']].T
        detect = np.loadtxt(i, usecols=(0, 2*joint_id+1,2*joint_id+2))[:flight.settings['num_detections']].T
