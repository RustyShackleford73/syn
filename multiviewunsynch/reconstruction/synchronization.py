...
def sync_bf(fps1, fps2, detect1, detect2, frame1, frame2, r=10):
    ...
    # multi-stage search
    t = time()
    search_space = r*fps2*2
    gap = fps2*4
    while search_space > 1.5*fps2:
        search_space = np.ceil(search_space/2)
        if search_space < fps2*1.5:
            gap = 1
        else:
            gap = np.ceil(gap/2)
        beta_iter = np.arange(beta_prior-search_space, beta_prior+search_space, gap)
        beta_est, numInlier = search(beta_iter)
        print(beta_est, beta_prior-search_space, beta_prior+search_space, np.ceil(gap))
    print((time()-t))
